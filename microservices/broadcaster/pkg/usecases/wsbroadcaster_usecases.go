...
type WSBroadcaster interface {
	...
	SendMessageToWS(message []byte)
	ReadClientMessage(conn *websocket.Connection)
...
func (wsb *DefaultWSBroadcaster) SendMessageToWS(message []byte) {
	...
	for client := range wsb.clients {
		...
		if err != nil {
			hasErrors = true
			wsb.logger.WithFields(log.Fields{
				usecases.SubscriberTag: client.RemoteAddr(),
				usecases.MessageTag:    message,
				usecases.ErrorTag:      err.Error(),
...
func (wsb *DefaultWSBroadcaster) ReadClientMessage(conn *websocket.Connection) {
	wsb.NewClient(conn)
	for {
...
func (wsb *DefaultWSBroadcaster) NewClient(conn *websocket.Connection) {
	...
	wsb.logger.WithFields(log.Fields{
		usecases.SubscriberTag: conn.RemoteAddr(),
